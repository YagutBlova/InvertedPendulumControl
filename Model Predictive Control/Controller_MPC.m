...
%% Alternative parameterization (used for the nonlinear model)
...
% Slightly modified versions used in control law
b3_hat = inert/N01_sq+0.1;
b4_hat = -N_val/N01_sq+0.1;
...
Disturbance = 1; % Enable random disturbances

%% Controller design (Explicit MPC)
Q = diag([1, 1, 1e7, 1e2]);  % State penalty
R = 1e7;                     % Input penalty
Np = 10;                     % Prediction horizon

% Compute MPC cost matrices
[H, F] = Controller_MPC_CostFunction(A, B, Np, Q, R, P);

% Vector to select first control move in explicit MPC
s = zeros(1, Np); 
s(1,1) = 1;
