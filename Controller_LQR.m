...
% Reference values (not directly used in LQR here)
q_r  = [0; 0];
qd_r = zeros(2,1);
...
%% Nonlinear pendulum model parameters
...
Disturbance = 1; % Enable disturbances

%% LQR Controller
Q = diag([1, 1, 1e7, 1e2]); % State weighting
R = 1e7;                    % Input weighting
[K, P] = dlqr(A, B, Q, R);  % Compute discrete LQR gain

%% Simulation loop
for k = 1:time/Ts-1

    % --- Control law (LQR) ---
    tau(1,k) = -K*[q(1,k); qd(1,k); q(2,k); qd(2,k)];
